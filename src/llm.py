...
from .vision_llm import analyze_image_with_gemma, crop_signature, verify_plant_candidate


RULES = (
    "You are an assistive smart-glasses voice assistant for older adults, with little dimentia. "
...
def plant_response_from_id(plant_id, config=None):
    ...
    details = _plant_summary(plant_id)
    if details:
        return clean_spoken_answer(f"{lead}. PlantNet reports {details}.")
    return clean_spoken_answer(f"{lead}.")
...
def _crop_cache_key(kind, intent, crop, ocr_text=""):
    return f"{kind}|{getattr(intent, 'value', intent)}|{ocr_text or ''}|{crop_signature(crop)}"
...
def _identify_plant_verified(crop, config):
    if getattr(config, "verify_plant_before_plantnet", True):
        verdict = verify_plant_candidate(crop, config)
        min_conf = float(getattr(config, "plant_verify_min_confidence", 0.70))
        is_plant = bool(verdict.get("is_plant"))
        confidence = float(verdict.get("confidence") or 0.0)
        if not is_plant or confidence < min_conf:
            reason = (verdict.get("reason") or "").strip()
            if reason:
                return None, clean_spoken_answer(
                    f"This does not look like a plant from here. {reason}"
                )
            return None, get_unclear_plant_response()
    return identify_plant(crop, config), None


def _sign_answer_from_vision(crop, intent, config, state, ocr_text, cache_key):
    ...
    visible_text = ((image_analysis or {}).get("visible_text") or ocr_text or "").strip()
    if intent == Intent.READ_SIGN_TEXT and len(visible_text) >= 4:
        answer = clean_spoken_answer(f"The sign says: {visible_text}")
...
def answer_for(kind, crop, intent, config=None, state=None):
    ...
    cache_key = _crop_cache_key(kind, intent, crop)
    ...
    if kind == "sign":
        ...
        if easy_answer:
            if state is not None:
                ...
                state.last_plant_id_result = None
            _store_cached_response(state, cache_key, easy_answer)
            return easy_answer
        return _sign_answer_from_vision(crop, intent, config, state, ocr_text, cache_key)
    ...
    if kind == "plant":
        ...
        plant_id, reject_answer = _identify_plant_verified(crop, config)
        if reject_answer:
            if state is not None:
                state.last_plant_id_result = None
                state.last_vision_result = None
            _store_cached_response(state, cache_key, reject_answer)
...
def describe_current_object(crop, config, state=None):
    cache_key = _crop_cache_key("object", Intent.WHAT_AM_I_LOOKING_AT, crop)
    cached = _get_cached_response(state, cache_key, ttl_seconds=120)
    if cached:
        return cached
