...
def classify_intent_fallback(text, detected_kind=None):
    ...
    if detected_kind == "plant" and has(text, r"\b(what is it|what is that|plant|flower|leaf|leaves)\b"):
        return Intent.EXPLAIN_PLANT
    if has(text, r"\b(why not|yes|yeah|yep|sure|okay|ok|go ahead|please|tell me|explain|can you do it|do it)\b"):
...
Rules:
- yes/sure/why not/sure why not/go ahead/okay/yes please/can you do it/do it -> confirmation
